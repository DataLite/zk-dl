...
//DATALITE patch				if(((BinderImpl)binding.getBinder()).hasValidator(binding.getComponent(), binding.getFieldName())){
					clearValidationMessages(binding.getBinder(),binding.getComponent(),binding.getFieldName());
//DATALITE patch				}
				
				ValidationContext vctx = new ValidationContextImpl(null, p, toCollectedProperties(p), ctx, true);

				if (binding.hasValidator()) {// DATALITE patch: perform standard ZK validation
					binding.validate(vctx);
				}
				// DATALITE patch: perform bean validation
